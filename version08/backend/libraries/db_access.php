<?php
   class db_access {
      ...
      function __construct($host, $user, $pass, $db = null, $reportar = MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT) {
         mysqli_report($reportar);
      ...
      private function escape_value($valor) {
         if ($valor === null) {
            ...
            foreach ($valor as &$actual) {
               $actual = $this->escape_value($actual);
            }
            return '"'.implode(', ', $valor).'"';
         } else if (is_object($valor) || is_string($valor)) {
            return '"'.$this->handle->escape_string((string)$valor).'"';
         } else if (is_bool($valor)) {
            return (int)$valor;
         } else if (is_int($valor) || is_float($valor)) {
            return $valor;
         } else {
            throw new Exception('db_access::escape_value');
         }
      }
      ...
      private function make_query($query, ...$parametros) {
         ...
         if (count($placeholders) == count($parametros)) {
            ...
            throw new Exception('db_access::make_query');
         }
      }
